...
*/
		...
		get() {
			return function () {
				let steps = this.getTutorial().getGenerator(),
					lighten = document.getElementsByClassName(this.constructor.lighten());

				for (let item = 0; item < lighten.length; item++) lighten[item].classList.remove(this.constructor.lighten());
		...
		play() {
			...
			while (true) {
				...
				if (next instanceof window.Bumblebee.Tutorial.Step) {
					...
					step.value.getBubble().getSpeech().getAudio().addEventListener('ended', next.get().bind(next), false);
				} else {
					step.value.getBubble().getSpeech().getAudio().addEventListener('ended', this.stop.bind(this), false);
				}
				if (i++ === 0)
	...
	class Bumblebee {
		...
		show(event) {
			...
			this.constructor.activete(event.target);
		...
		hide(event) {
			...
			this.constructor.deactivete(event.target);
			this.getBubble().hide();
		}
		handleEvent(event) {
		...
		static activete(element) {
			element.classList.add(window.Bumblebee.lighten());
		}
		static deactivete(element) {
